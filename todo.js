jQuery(document).ready(function($){
    ...
    var complete = {
        load: function(tasks) {
            ...
            $.each(tasks,function(i,x) {
                completed += "<li><a href='#' class='remove'>X</a> <span id='"+tasks[i].id+"' class='task'>"+tasks[i].task+"</span></li>";
            });
            completed += "</ul>";
            completed += "</div>";
            $(".app").html(completed);
    ...
    var handlers = {
        load: function() {
            $(".add-task").on("click", function() {
                ...
                $(".incomplete.task-list > ul").append("<li><a href='#' class='complete'>&#x2713</a> <a href='#' class='remove'>X</a> <span id='"+todo.count()+"' class='task'>"+task+"</span></li>");
                todo.save(task);
                $(".new-task").val("");
            });
